function initPolice(gl) {
    ...
    var cap_height = CAP_HEIGHT + 0.001;
    var cap_width = width + 0.001;
    var cap_length = height + 0.001;
    ...
    const head_array = [
      // Front face
       ...
       cap_width,  cap_height + TORSO_HEIGHT + HEAD_HEIGHT + 2*CAP_HEIGHT, cap_length,
        cap_width, -cap_height + TORSO_HEIGHT + HEAD_HEIGHT + 2*CAP_HEIGHT, cap_length,
       -cap_width,  cap_height + TORSO_HEIGHT + HEAD_HEIGHT + 2*CAP_HEIGHT, cap_length,
       -cap_width, -cap_height + TORSO_HEIGHT + HEAD_HEIGHT + 2*CAP_HEIGHT, cap_length,
  ...
  function drawPolice(gl, programInfo, player, deltaTime, projectionMatrix, texture){
    
    var seconds = new Date().getTime() / 1000;
    
    rotation_angle = Math.PI/2.0 * Math.sin(seconds * 10);

    draw_parts_police(gl, programInfo, projectionMatrix, player.position, player.buffer.position, player.buffer.color, player.buffer.indices, 36, null)
    draw_parts_police(gl, programInfo, projectionMatrix, player.position, player.buffer.hand, player.buffer.color_hand, player.buffer.indices_hand, 36, [-1.0,1.0,0.0])
    draw_parts_police(gl, programInfo, projectionMatrix, player.position, player.buffer.hand_2, player.buffer.color_hand_2, player.buffer.indices_hand_2, 36, [1.0,1.0,0.0])
    draw_parts_police(gl, programInfo, projectionMatrix, player.position, player.buffer.leg, player.buffer.color_leg, player.buffer.indices_leg, 36, [1.0,0.0,0.0])
    draw_parts_police(gl, programInfo, projectionMatrix, player.position, player.buffer.leg_2, player.buffer.color_leg_2, player.buffer.indices_leg_2, 36, [-1.0,0.0,0.0])
    draw_parts_police(gl, programInfo, projectionMatrix, player.position, player.buffer.head, player.buffer.color_head, player.buffer.indices_head, 36 + 36, null)
  
  }


  function draw_parts_police(gl, programInfo, projectionMatrix, translate_position, vertices, colors, indices, vertex_count, rotation_axis){
    ...
    mat4.scale(modelViewMatrix,modelViewMatrix,scale_vector);
